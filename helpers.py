...
def construct_graph(LAMBDA=0, k=10, lr=0.001, BATCH_SIZE=1024*16, n_users=138493, n_movies=27278):
    ...
    UV_xft = tf.Variable(tf.truncated_normal(shape=(k,k), mean=-1./k, stddev=0.2), dtype=tf.float32)
    UU_xft = tf.Variable(tf.truncated_normal(shape=(k,k), mean=-1./k, stddev=0.2), dtype=tf.float32)
    VV_xft = tf.Variable(tf.truncated_normal(shape=(k,k), mean=-1./k, stddev=0.2), dtype=tf.float32)
