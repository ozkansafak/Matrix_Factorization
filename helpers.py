...
def construct_graph(LAMBDA=0, k=10, lr=0.001, BATCH_SIZE=1024*16, n_users=138493, n_movies=27278):
    ...
    X_UV = tf.Variable(tf.truncated_normal(shape=(k,k), mean=-1./k, stddev=0.2), dtype=tf.float32)
    ...
    xft = X_UV[0,1] * tf.multiply(tf.transpose(stacked_U)[0], tf.transpose(stacked_V)[1])
    for p in range(k):
        for q in range(0, k):
            xft += X_UV[p,q] * tf.multiply(tf.transpose(stacked_U)[p], tf.transpose(stacked_V)[q])
